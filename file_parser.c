...
// Struct for M-Type instructions mapping for the 'function' field in the instruction
struct {
	const char *name;
	char *function;
} mMap[] = {
...
void parse_file(FILE *fptr, int pass, char *instructions[], size_t inst_len, hash_table_t *hash_table, FILE *Out) {
	...
	int32_t instruction_count = 0x00;
	int data_reached = 0;
	//FILE *fptr;
	...
	while (1) {
		...
		while (1) {
			...
			/*
			 * If token is "li", increment by 2, otherwise if it exists in instructions[],
			 * increment by 1.
			 */
			int x = search(token);
						...
						*/
						...
						if (inst_type == 'i') {
							...
							*/
							...
							// I-Type rt, rs, im
							else if ( strcmp(token, "slti") == 0 || strcmp(token, "addi") == 0) {
						...
						//******************add implementation (I-type)************
						//******************li implementation (M-type)*************
						// J-Type
						else if (inst_type == 'j') {
...
char instruction_type(char *instruction) {
	...
	if (strcmp(instruction, "lw") == 0 || strcmp(instruction, "sw") == 0
			|| strcmp(instruction, "add") == 0 || strcmp(instruction,
